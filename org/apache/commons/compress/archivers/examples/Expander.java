...
public class Expander
{
    ...
    public void expand(final TarFile archive, final File targetDirectory) throws IOException, ArchiveException {
        ...
        final Iterator<ArchiveEntry> iterator;
        final InputStream in;
        final Throwable t2;
        this.expand(() -> iterator.hasNext() ? iterator.next() : null, (entry, out) -> {
    ...
    public void expand(final ZipFile archive, final File targetDirectory) throws IOException, ArchiveException {
        ...
        final Enumeration<ZipArchiveEntry> enumeration;
        ZipArchiveEntry next;
        final InputStream in;
        final Throwable t2;
        this.expand(() -> {
            for (next = (enumeration.hasMoreElements() ? enumeration.nextElement() : null); next != null && !archive.canReadEntryData(next); next = (enumeration.hasMoreElements() ? enumeration.nextElement() : null)) {}
            return next;
        }, (entry, out) -> {
            in = archive.getInputStream(entry);
            try {
